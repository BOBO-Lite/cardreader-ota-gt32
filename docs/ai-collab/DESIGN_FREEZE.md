...
2. **Secondary 整槽（2×D8h）墙钟一次实测** — 单位均值已登记（见下「已关闭/部分」）；整槽 D8h 实走 Boot 路径仍建议补测一次，便于 AC-YM-05 升格  

**已部分登记（AC-YM-05 墙钟，2026-09-12，来源：用户确认）：** 写 1 KiB 均值 **18 ms**；擦 1 KiB 均值 **200 ms**。线性外推 Secondary 128 KiB ≈ **25.6 s**，相对默认「头 ACK 后等 C」**30 s** 仍兼容（余量约 4.4 s，偏紧）。**维持 30 s，不上调发送端**。本设计预擦为 2×D8h，整槽通常更快；单位均值推算 ≠ 整槽 E3 通过。详见 DECISIONS D-TIM-001。  
